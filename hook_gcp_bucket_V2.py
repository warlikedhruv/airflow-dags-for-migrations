...
#     avro_files_list_standard_feed = []
#     avro_files_list_kv_feed = []
#     avro_files_filtered = {}
#     for file in manifest_json_files_list:
#         try:
#             data = connection.download(bucket_name="airflow-test-bucket-1107", object_name=file)
#             data = json.loads(data)
#             if "standard_feed " in data['feed_type']:
#                 avro_files_list_standard_feed.extend([filepath for filepath in data['filepath']])
#             elif "auction_kv_labels_feed" in data['feed_type']:
#                 avro_files_list_kv_feed.extend([filepath for filepath in data['filepath']])
#         except exceptions.NotFound as e:
#             logger = logging.getLogger("airflow.task")
#             logger.error("Exception Not found:" + str(e))
#
#     avro_files_filtered['standard_feed'] = avro_files_list_standard_feed
#     avro_files_filtered['auction_kv_labels_feed'] = avro_files_list_kv_feed
#     return avro_files_filtered
...
def process_manifest_json_file(**kwargs):
    ...
    for file in manifest_json_files_list:
        try:
            data = connection.download(bucket_name="airflow-test-bucket-1107", object_name=file)
            data = json.loads(data)
            hour = datetime.strptime(data["hour"], "%Y-%m-%d %H:%M:%S")

            if hour < yesterday_date:
                break
            path = data['path'][1:]
            response.extend([{data['feed_type']: path + "/" +  filepath['name'] } for filepath in data['files']])
        except Exception as e:
            print(e)
    if len(response) < 1:
        raise AirflowSkipException
...
#     valid_avro_standard_files_bq = []
#     valid_avro_files_archive = []
#
#     for key in avro_files_filtered.keys():
#         for file in avro_files_filtered[key]:
#             if connection.exists(bucket_name=bucket_name, object_name=file):
#                 valid_avro_files_archive.append(file)
#                 if key == "standard_feed":
#                     valid_avro_standard_files_bq.append("gs://" + bucket_name + "/" + file)
#
#                 elif key == "auction_kv_lables_feed":
#                     pass
#
#     return {"valid_files_bq": valid_avro_standard_files_bq, "valid_files_archive": valid_avro_files_archive}
...
def check_valid_avro_filepath(**kwargs):
    ...
    avro_files_filtered = xComm_var.xcom_pull(task_ids="process_manifest_json_file")
    valid_files_bq = []
    valid_files_bq_archive = []
    for data in avro_files_filtered:
        for key in data.keys():
            if connection.exists(bucket_name=bucket_name, object_name=data[key]):
                valid_files_bq.append({key: "gs://" + bucket_name + "/" + data[key]})
                valid_files_bq_archive.append(data[key])
    return {"valid_files_bq": valid_files_bq, "valid_files_archive": valid_files_bq_archive}
...
def load_bq(**kwargs):
    ...
    schema_name = {
        "auction_kv_labels": "schema_name_auction_kv_labels",
        "standard_feed": "schema_name_standard_feed"
    }
    valid_avro_files = valid_avro_file['valid_files_bq']
...
def archive_files(**kwargs):
    ...
    xComm_var = kwargs['ti']
    valid_avro_file = xComm_var.xcom_pull(task_ids='check_valid_avro_filepath')
    valid_avro_files_archive = valid_avro_file['valid_files_archive']
    print(valid_avro_file)
    for file in valid_avro_files_archive:
        file_name = file.split("/")[-1]
        connection.copy(
